...
json parse_options(bool streaming, const backend::PredictOptions* predict, server_context &llama)
{
...
static void start_llama_server() {
    ...
    llama.queue_tasks.on_update_slots(std::bind(
...
public:
    ...
    grpc::Status PredictStream(grpc::ServerContext* context, const backend::PredictOptions* request, grpc::ServerWriter<backend::Reply>* writer) override {
            ...
            llama.request_completion(task_id, -1, data, false, false);
            while (true)
            {
                ...
                if (!result.error) {
                    ...
                    backend::Reply reply;
                    // print it
                    std::string completion_text = result.data.value("content", "");
                    
                    reply.set_message(completion_text);
                    int32_t tokens_predicted = result.data.value("tokens_predicted", 0);
                    reply.set_tokens(tokens_predicted);
                    int32_t tokens_evaluated = result.data.value("tokens_evaluated", 0);
    ...
    grpc::Status Predict(ServerContext* context, const backend::PredictOptions* request, backend::Reply* reply) {
        ...
        if (!result.error && result.stop) {
            completion_text = result.data.value("content", "");
            int32_t tokens_predicted = result.data.value("tokens_predicted", 0);
            int32_t tokens_evaluated = result.data.value("tokens_evaluated", 0);
            reply->set_prompt_tokens(tokens_evaluated);
            reply->set_tokens(tokens_predicted);
    ...
    grpc::Status Embeddings(ServerContext* context, const backend::PredictOptions* request, backend::EmbeddingResult* response) override {
        json data = parse_options(false, request, llama);
        const int task_id = llama.queue_tasks.get_new_id();
        llama.queue_results.add_waiting_task_id(task_id);

        if (request->embeddingtokens().size() > 0){
            data["prompt"] = request->embeddingtokens();
        } else if (request->embeddings().size() > 0){
            data["prompt"] = request->embeddings();
        } else {
            return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT, "");
        }

        llama.request_completion(task_id, -1, data, false, true);
        server_task_result result = llama.queue_results.recv(task_id);      
        llama.queue_results.remove_waiting_task_id(task_id);
        if (!result.error) {
            auto emb = result.data["embedding"].get<std::vector<float>>();
            for (const auto& item : emb){
                response->add_embeddings(item);
            }
            return grpc::Status::OK;
        }
        else
...
void RunServer(const std::string& server_address) {
  ...
  std::cout << "Llama.cpp gprc Server listening on " << server_address << std::endl;
  server->Wait();
}
